...
#if [ "$LOGROTATE" = "true" ] && [ "$GOA" = "false" ]; then while true; do sleep $LOGROTATETIME; logrotate --verbose --state /data/etc/logrotate.status /etc/logrotate; done; fi &
#if [ "$LOGROTATE" = "true" ] && [ "$GOA" = "true" ]; then while true; do sleep $LOGROTATETIME; killall goaccess; sleep 10s; logrotate --verbose --state /data/etc/logrotate.status /etc/logrotate; done; fi &
if [ "$LOGROTATE" = "true" ]; then while true; do sleep $LOGROTATETIME; logrotate --verbose --state /data/etc/logrotate.status /etc/logrotate; done; fi &
