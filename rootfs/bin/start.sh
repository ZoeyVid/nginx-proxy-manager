...
sed -i "s|#    ssl_certificate /data/tls/dummycert.pem;|    ssl_certificate "$NPM_CERT";|g" /usr/local/nginx/conf/conf.d/include/default.conf
sed -i "s|#    ssl_certificate_key /data/tls/dummykey.pem;|    ssl_certificate_key "$NPM_KEY";|g" /usr/local/nginx/conf/conf.d/include/default.conf
if [ -n "$NPM_CHAIN" ]; then
sed -i "s|#    ssl_trusted_certificate;|    ssl_trusted_certificate "$NPM_CHAIN";|g" /usr/local/nginx/conf/conf.d/include/default.conf
fi

sed -i "s|#    ssl_certificate /data/tls/dummycert.pem;|    ssl_certificate "$NPM_CERT";|g" /usr/local/nginx/conf/conf.d/no-server-name.conf
sed -i "s|#    ssl_certificate_key /data/tls/dummykey.pem;|    ssl_certificate_key "$NPM_KEY";|g" /usr/local/nginx/conf/conf.d/no-server-name.conf
if [ -n "$NPM_CHAIN" ]; then
sed -i "s|#    ssl_trusted_certificate;|    ssl_trusted_certificate "$NPM_CHAIN";|g" /usr/local/nginx/conf/conf.d/no-server-name.conf
fi

sed -i "s|#    ssl_certificate /data/tls/dummycert.pem;|    ssl_certificate "$NPM_CERT";|g" /usr/local/nginx/conf/conf.d/npm.conf
sed -i "s|#    ssl_certificate_key /data/tls/dummykey.pem;|    ssl_certificate_key "$NPM_KEY";|g" /usr/local/nginx/conf/conf.d/npm.conf
if [ -n "$NPM_CHAIN" ]; then
sed -i "s|#    ssl_trusted_certificate;|    ssl_trusted_certificate "$NPM_CHAIN";|g" /usr/local/nginx/conf/conf.d/npm.conf
fi

sed -i "s|#  ssl_certificate /data/tls/dummycert.pem;|  ssl_certificate "$NPM_CERT";|g" /app/templates/default.conf
sed -i "s|#  ssl_certificate_key /data/tls/dummykey.pem;|  ssl_certificate_key "$NPM_KEY";|g" /app/templates/default.conf
if [ -n "$NPM_CHAIN" ]; then
sed -i "s|#  ssl_trusted_certificate;|  ssl_trusted_certificate "$NPM_CHAIN";|g" /app/templates/default.conf
fi
