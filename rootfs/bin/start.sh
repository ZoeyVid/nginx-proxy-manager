...
for phpv in $(find /etc/php* -maxdepth 0 | sed "s|/etc/php||g"); do cp -vrnT /etc/php"$phpv" /data/php/"$phpv" || sleep inf; done;
for phpv in $(find /etc/php* -maxdepth 0 | sed "s|/etc/php||g"); do sed -i "s|user =.*|user = root|" /data/php/"$phpv"/php-fpm.d/www.conf || sleep inf; done;
for phpv in $(find /etc/php* -maxdepth 0 | sed "s|/etc/php||g"); do sed -i "s|group =.*|group = root|" /data/php/"$phpv"/php-fpm.d/www.conf || sleep inf; done;
for phpv in $(find /etc/php* -maxdepth 0 | sed "s|/etc/php||g"); do sed -i "s|listen =.*|listen = /dev/php$phpv.sock|" /data/php/"$phpv"/php-fpm.d/www.conf || sleep inf; done;
for phpv in $(find /etc/php* -maxdepth 0 | sed "s|/etc/php||g"); do sed -i "s|include=.*|include=/data/php/$phpv/php-fpm.d/*.conf|g" /data/php/"$phpv"/php-fpm.conf || sleep inf; done;
